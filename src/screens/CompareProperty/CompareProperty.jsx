...
const Panel = ({ text, options, iconSrc, value , name ,handleChangeFilter }) => {
  return (
    <div className="flex text-black  items-center  bg-[#F5F7F8]  rounded-[25px] cursor-pointer p-[12px]">
      {/* Wrap select in a label */}
      <label>
        <select name={name} value={value} onChange={handleChangeFilter} className="bg-transparent text-[14px] font-light focus:outline-none focus:border-blue-500 cursor-pointer">
    ...
    <Layout title={"Search & Compare"}>
      <div className="[font-family:Poppins,Helvetica] font-bold h-[47px] my-6 flex justify-between gap-[10px]">
        <div className="w-[25%] h-[47px] bg-[#f6f6fa] rounded-[23.5px] flex items-center p-[5px]">
          ...
          placeholder="Search..."
          />
          <button onClick={handleSubmit} className="bg-white active:scale-95 p-[12px] rounded-full w-[41px] h-[41px] drop-shadow-md">
            <img
        ...
        <div className="relative w-[124px] h-[47px]">
          <button onClick={handleSubmit} className="relative active:scale-95 w-[122px] h-[47px] bg-black  rounded-[23.5px]">
